...
import { EditPackageNameButton, EmptyState, PackageNameContainer, PackageNameInput, TrackerContainer, TrackerInfo, TrackerInfoGeneral, TrackerList } from './styles'
          ...
          <>
            ...
            <TrackerInfoGeneral>
              ...
              <span className='last-update'>
                ...
                <PackageNameContainer>
                  <PackageNameInput
                    ref={inputRef}
                    value={packageName}
                    style={{ pointerEvents: isEditingPackageName ? 'all' : 'none' }}
                    placeholder="Escreva o nome"
                    maxLength={15}
                    onChange={e => setPackageName(e.target.value)}
                  />
                  <EditPackageNameButton>
                    {isEditingPackageName ? packageName && (
                      <BiSave onClick={() => handleSavePackageName(trackingResults?.codigo)} />
                    ) : (
                      <BiPencil onClick={() => {
              ...
              {trackingResults?.codigo && <span className="last-update">
                Código: <br /><strong>{trackingResults?.codigo}</strong>
              </span>}
